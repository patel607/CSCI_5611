...
void draw () {
  ...
  surface.setTitle("GALAGA" + "   FPS: " + str(frameRate));
}
...
void handleHit() {
  ...
  for (int i = 0; i < enemyBullet.size(); i++) {
    if (enemyBullet.get(i).pos.y > height)
      enemyBullet.remove(enemyBullet.get(i));
    else if (dist(enemyBullet.get(i).pos.x, enemyBullet.get(i).pos.y, s.pos.x, s.pos.y) < s.radius) {
      s.hits++;
      enemyBullet.remove(enemyBullet.get(i));
      if (s.hits >= s.health)
        explosions.add(new Explosion(s.pos));
